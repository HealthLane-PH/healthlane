...
import { notify } from "@/components/ToastConfig"; // ✅ Added for toast
...
export default function Sidebar({ onClose }: { onClose?: () => void }) {
  ...
  useEffect(() => {
    const unsubscribe = auth.onAuthStateChanged(async (u) => {
      if (u) {
        ...
        // 🔍 Try fetching from "patients" (not users)
        const userRef = doc(db, "patients", u.uid);
        ...
        if (snap.exists()) {
          ...
          const q = query(collection(db, "patients"), where("email", "==", u.email));
  ...
  const menuItems = [
    { name: "Home", path: "/dashboard", icon: <HomeIcon className="h-5 w-5" /> },
    { name: "Profile", path: "/dashboard/profile", icon: <UserCircleIcon className="h-5 w-5" /> },
    { name: "Settings", path: "/dashboard/settings", icon: <Cog6ToothIcon className="h-5 w-5" /> },
    { name: "Alerts", path: "/dashboard/alerts", icon: <BellIcon className="h-5 w-5" /> },
  ];
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              <p className="font-medium text-gray-800 truncate">
                {userData?.preferredName || userData?.firstName || "User"}
              </p>
