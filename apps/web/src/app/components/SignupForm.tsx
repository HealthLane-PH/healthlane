...
export default function SignupForm({
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await setDoc(doc(db, "users", user.uid), {
        ...
        isVerified: false,                  // email verification status (app-level)
        profileStatus: selectedRole === "doctor" ? "pending" : "active",
  ...
  const handleGoogle = async () => {
    try {
      await googleSignIn(role);   // 👈 pass the role coming from props
      router.push("/dashboard");  // 👈 guard will route to onboarding or main page
    } catch (err: any) {
      alert(err.message);
    }
  };
